...
### AWS Secure Token Service (STS)
  * role can be assumed by invoking said service
  * provides temporary credentials for a role with a maximum age of 12 hours
  * when role adopted,  previous permissions are gone
  * roles invoked for
    * delegating cross-account access
    * access a resource that doesn’t usually fall into their use-case within their account
  * not a scalable identity federation procedure
   * if wanting to explicitly reference and use cross-account resources in your own account, a user should instead utilize a resource-based policy to grant the relevant permission instead, which said user must have an identity-based policy explicitly granting permission and the cross-account resource must have an explicit resource-based policy granting permission to the user.  These policies are generally used when wanting to directly share resources with another one of your accounts and control the integration of those resources into the account’s environment. 
  * interacted with either by:
    * swapping roles in the AWS Management Console
    * utilizing a url generated by the STS API
    * invoking the STS API manually to generate temporary security credentials
  * Need to perform the following steps if account A is to have access to a resource in account B through a role.
    1. Create a role in Account B. During creation, attach a policy that explicitly allows everything you want the user in Account A to be able to do. Recall that the user will lose all his permissions when he assumes this role, so you must exhaustively list everything. Then, record the account number of Account A as a trusted account ID. When created, take note of the ARN (Amazon Resource Name) of the role.
    2. Attach a policy to the user in Account A that explicitly allows them to invoke the AssumeRole API call in STS for the ARN of the role created in Account B.
    3. The user in account A should now be able to invoke STS to access the resources he requires in account B.
  * An example use case being a data storage resource containing log data in a ‘production’ AWS account and a security research AWS account that would perform analysis on that log data. A resource-policy would be attached to the data storage resource and an identity-policy to the ‘security research’ group in the security AWS account. In both of these policies, you would specify that the security research group is only allowed to read data from the specific storage resource containing the log data.
...
### Route 53 Records:
  ...
  * Private hosted zones=>traffic to VPC(s)
    * For each associated VPC with a Route 53 Hosted Zone, to achieve DNS resolution, the following VPC settings need to be set to true: enableDnsHostnames and enableDnsSupport
...
#### EC2 Spot Instance:
  ...
  * A spot instance request is either *one-time* or *persistent*, with the latter automatically resubmitted after a Spot Instance associated with the request is terminated.  A spot instance request can optionally specify a duration for the instance(s).  If a request is persistent and you stop the Spot Instance, the request only opens after you start the Spot Instance
...
### AWS Systems Manager (SSM)
  ...
  * Use the Run Command to execute commands/scripts to be run at scale on EC2 instances (eg: patching)
  * Configure Systems Manager Maintenance Windows to coordinate patching and instance removal from the ALB during the defined window, which can run Automation documents (eg: AWSEC2-PatchLoadBalancerInstance document) or Lambda functions at precise times. You can schedule a task to remove instances from the load balancer using pre-defined documents (such as AWSEC2-PatchLoadBalancerInstance) and then re-register them once patching is complete. This offers fine-grained scheduling and orchestration for controlled patching without disrupting traffic.
  * Use AWS Systems Manager Automation with the AWSEC2-PatchLoadBalancerInstance document to manage patching, which is specifically designed for patching EC2 instances that are part of a load balancer. It automatically removes the instance from the ALB target group, waits for in-flight requests to complete, applies patches, performs reboots if needed, and then safely re-registers the instance. This workflow prevents application downtime or request failures during patching and ensures compliance with security policies.
...
### AWS Glue:
  ...
  * Glue Databrew-clean/normalize data visually using pre-built transformation (more apt for non-techical stakeholders as it is code-free) that offers lineage tracking (auditability/traceability), data profiling for column inspection statistics, null values, outliers, std dev, and data types across datasets
